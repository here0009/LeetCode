...
class Solution:
    def visiblePoints(self, points, angle: int, location) -> int:
        ...
        for i in range(len(points)):
            if points[i][1] == location[1] and points[i][0] == location[0]:
                overlap += 1
            else:
                a = atan2(points[i][1]-location[1], points[i][0]-location[0])
                angle_list.append(a)
                angle_list.append(a+2*pi)
        angle_list.sort()
        # print(angle_list, angle)
        res = 0
        dq = deque([])
        for a in angle_list:
            while dq and a-dq[0] > angle:
                dq.popleft()
            dq.append(a)
            res = max(res, len(dq))
        return res + overlap
