...
class Solution:
    def areConnected(self, n: int, threshold: int, queries):
        """
        wrong answer, connections through intermedia nodes are still conncetions, such as 9->18->12->16->24
        """
        @lru_cache(None)
        def gcd(m, n):
            if m % n == 0:
...
threshold = 2
queries = [[1,4],[2,5],[3,6]]
print(S.areConnected(n, threshold, queries))
n = 6
threshold = 0
