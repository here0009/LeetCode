...
"""
Thoughts:
if there is a loop, then you can not finish all the courses
topology sort..
"""
class Solution_1:
...
class Solution:
    def canFinish(self, numCourses: int, prerequisites) -> bool:
        """
        visited will change back to 0, when the dfs complete,
        if a node is checked, checked[i] will change to  1
        """
        def dfs(i):
            for j in edges[i]:
                if visited[j] == 1:
