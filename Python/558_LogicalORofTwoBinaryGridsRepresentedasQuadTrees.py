...
class Solution:
    def intersect(self, quadTree1: 'Node', quadTree2: 'Node') -> 'Node':
        ...
        if (quadTree1.isLeaf and quadTree1.val == 0) and (quadTree2.isLeaf and quadTree2.val == 0):
            return Node(0, True, None, None, None, None)
        # one of quadTree1, quadTree2 is not leaf
        topLeft = self.intersect(quadTree1.topLeft, quadTree2.topLeft)
        topRight = self.intersect(quadTree1.topRight, quadTree2.topRight)
        bottomLeft = self.intersect(quadTree1.bottomLeft, quadTree2.bottomLeft)
        bottomRight = self.intersect(quadTree1.bottomRight, quadTree2.bottomRight)
        values = set([topLeft.val, topRight.val, bottomLeft.val, bottomRight.val])
        isLeaf = bool(len(values) == 1) and sum([topLeft.isLeaf, topRight.isLeaf, bottomLeft.isLeaf, bottomRight.isLeaf]) == 4
        val = 0
        if isLeaf:
            val = values.pop()
