...
class Solution:
    def minOperationsMaxProfit(self, customers, boardingCost: int, runningCost: int) -> int:
        rotation, curr, wait = 0, 0, 0
        n = len(customers)
        res, res_index = -float('inf'), -1

        while rotation < n or wait > 0:
            if rotation < n:
                wait += customers[rotation]
            if wait > 0:
                curr += min(4, wait)
                wait = max(0, wait-4)
            tmp = boardingCost*curr - runningCost*(rotation+1)
            if tmp > res:
                res = tmp
                res_index = rotation+1
            rotation += 1

        return res_index if res > 0 else -1
