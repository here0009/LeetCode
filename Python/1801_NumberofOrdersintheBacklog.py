...
class Solution:
    def getNumberOfBacklogOrders(self, orders: List[List[int]]) -> int:
        ...
        for price, amount, orderType in orders:
            if orderType == 0:  # buy
                while sell_orders and amount > 0 and sell_orders[0][0] <= price:
                    deal = min(sell_orders[0][1], amount)
                    amount -= deal
                    sell_orders[0][1] -= deal
                    if sell_orders[0][1] == 0:
                        heapq.heappop(sell_orders)
                if amount > 0:
                    heapq.heappush(buy_orders, [-price, amount])
            else:  # sell
                while buy_orders and amount > 0 and -buy_orders[0][0] >= price:
                    deal = min(buy_orders[0][1], amount)
                    amount -= deal
                    buy_orders[0][1] -= deal
                    if buy_orders[0][1] == 0:
        ...
        res = sum(k for _, k in buy_orders) + sum(k for _, k in sell_orders)
        return res % M
