...
class Solution:
    def getTriggerTime(self, increase, requirements):
        ...
        res = []
        length = len(A)
        for ra,rb,rc in requirements:
            index_a = bisect_left(A, ra)
            index_b = bisect_left(B, rb)
            index_c = bisect_left(C, rc)
            index = max(index_a,index_b,index_c)
            res.append(-1 if index == length else index)
        return res
