        ...
        def dp(i,j,k):
            ...
            if length < k:
                ...
                for left_k in range(1,k):
                    ...
                    res = min(res, max(jobDifficulty[i:i+leftX])+dp(i+left,j,k-left))
            return res
...
class Solution:
    def minDifficulty(self, jobDifficulty, d: int) -> int:
        ...
        def dp(index, k):
            """
            min difficulty of schedule jobDifficulty[index:] to k days
            """
            res = float('inf')
            if length-index < k:
                return res
            ...
            if length-index == k:
                return sum(jobDifficulty[index:])
            curr_max = jobDifficulty[index]
            i = 0
            while length-(index+i) > k-1:
                curr_max = max(curr_max, jobDifficulty[index+i])
                res = min(res, curr_max+dp(index+i+1,k-1))
                i += 1
            return res
