...
class Solution:
    def stoneGameVI(self, aliceValues: List[int], bobValues: List[int]) -> int:
        ...
        nums = [(aliceValues[i] + bobValues[i], i) for i in range(length)]
        nums.sort(reverse=True)
        a, b = 0, 0
        for i in range(length):
            if i % 2 == 0:
                a += aliceValues[nums[i][1]]
            else:
                b += bobValues[nums[i][1]]
        # print(nums)
        if a > b:
            return 1
        elif a == b:
