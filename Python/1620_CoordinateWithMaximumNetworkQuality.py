"""
You are given an array of network towers towers and an integer radius, where towers[i] = [xi, yi, qi] denotes the ith network tower with location (xi, yi) and quality factor qi. All the coordinates are integral coordinates on the X-Y plane, and the distance between two coordinates is the Euclidean distance.

The integer radius denotes the maximum distance in which the tower is reachable. The tower is reachable if the distance is less than or equal to radius. Outside that distance, the signal becomes garbled, and the tower is not reachable.

The signal quality of the ith tower at a coordinate (x, y) is calculated with the formula ⌊qi / (1 + d)⌋, where d is the distance between the tower and the coordinate. The network quality at a coordinate is the sum of the signal qualities from all the reachable towers.

Return the integral coordinate where the network quality is maximum. If there are multiple coordinates with the same network quality, return the lexicographically minimum coordinate.

Note:

A coordinate (x1, y1) is lexicographically smaller than (x2, y2) if either x1 < x2 or x1 == x2 and y1 < y2.
⌊val⌋ is the greatest integer less than or equal to val (the floor function).
 

Example 1:


Input: towers = [[1,2,5],[2,1,7],[3,1,9]], radius = 2
Output: [2,1]
Explanation: 
At coordinate (2, 1) the total quality is 13
- Quality of 7 from (2, 1) results in ⌊7 / (1 + sqrt(0)⌋ = ⌊7⌋ = 7
- Quality of 5 from (1, 2) results in ⌊5 / (1 + sqrt(2)⌋ = ⌊2.07⌋ = 2
- Quality of 9 from (3, 1) results in ⌊9 / (1 + sqrt(1)⌋ = ⌊4.5⌋ = 4
No other coordinate has higher quality.
Example 2:

Input: towers = [[23,11,21]], radius = 9
Output: [23,11]
Example 3:

Input: towers = [[1,2,13],[2,1,7],[0,1,9]], radius = 2
Output: [1,2]
Example 4:

Input: towers = [[2,1,9],[0,1,9]], radius = 2
Output: [0,1]
Explanation: Both (0, 1) and (2, 1) are optimal in terms of quality but (0, 1) is lexicograpically minimal.
 

Constraints:

1 <= towers.length <= 50
towers[i].length == 3
0 <= xi, yi, qi <= 50
1 <= radius <= 50
"""


from collections import Counter
from collections import deque
from math import sqrt
class Solution:
    def bestCoordinate(self, towers, radius: int):
        """
        wrong answer, did not caluculate all the reachable points on the grid
        """
        dir4 = [(0,1),(0,-1),(1,0),(-1,0)]
        dir8 = [(-1,1),(-1,-1),(1,-1),(1,1)]
        quality = Counter()
        for x,y,q in towers:
            quality[(x,y)] += q
        d = 1
        print(quality)
        while d <= radius:
            print(d,'dir4')
            for x,y,q in towers:
                for di,dj in dir4:
                    ni,nj = x + d*di, y + d*dj
                    quality[(ni,nj)] += int(q/(1+d))
            
            d += 1
            print(quality)
        d = 1
        while sqrt(2)*d <= radius:
            print(d,'dir8')
            for x,y, q in towers:
                for di,dj in dir8:
                    ni,nj = x + d*di, y + d*dj
                    quality[(ni,nj)] += int(q/(1+sqrt(2)*d))
            d += 1
            print(quality)

        # for key,v in sorted(quality.items()):
        #     print(key, v)
        res_val = 0
        res_key = (0,0)
        for key, val in quality.items():
            if val > res_val or (val == res_val and key < res_key):
                res_val = val
                res_key = key
        return list(res_key)


class Solution:
    def bestCoordinate(self, towers, radius: int):
        def getScore(i,j):
            res = 0
            for x,y,q in towers:
                d = sqrt((x-i)**2 + (y-j)**2)
                if d <= radius:
                    res += int(q/(1+d))
            return res

        minX, minY, maxX, maxY = float('inf'), float('inf'), -float('inf'), -float('inf')
        for x,y,q in towers:
            minX = min(minX, x)
            maxX = max(maxX, x)
            minY = min(minY, y)
            maxY = max(maxY, y)
        res = None
        maxQ = 0
        for i in range(minX, maxX+1):
            for j in range(minY, maxY+1):
                tmp = getScore(i,j)
                if tmp > maxQ:
                    maxQ = tmp
                    res = [i,j]
        return res



S = Solution()
towers = [[1,2,5],[2,1,7],[3,1,9]]
radius = 2
print(S.bestCoordinate(towers, radius))
towers = [[23,11,21]]
radius = 9
print(S.bestCoordinate(towers, radius))
towers = [[1,2,13],[2,1,7],[0,1,9]]
radius = 2
print(S.bestCoordinate(towers, radius))
towers = [[2,1,9],[0,1,9]]
radius = 2
print(S.bestCoordinate(towers, radius))
towers = [[44,31,4],[47,27,27],[7,13,0],[13,21,20],[50,34,18],[47,44,28]]
radius = 13
# Output:
# [47,44]
# Expected:
# [47,27]
print(S.bestCoordinate(towers, radius))
towers = [[32,36,27],[17,22,43],[8,11,41],[46,28,7],[22,4,35],[41,8,33],[32,29,4],[44,32,16],[33,20,16],[3,38,35],[17,47,23],[33,0,29],[29,19,6],[4,50,46],[19,47,6],[48,6,41],[20,26,35]]
radius = 4
print(S.bestCoordinate(towers, radius))