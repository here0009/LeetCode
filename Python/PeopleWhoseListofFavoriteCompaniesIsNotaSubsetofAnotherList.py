...
class Solution:
    def peopleIndexes(self, favoriteCompanies):
        ...
        for i,l in enumerate(favoriteCompanies):
            tmp_set = set(l)
            companies_dict[i] = tmp_set
        length = len(companies_dict)
        is_subset = [0]*length
        for i in range(length-1):
            for j in range(i+1, length):
                if is_subset[i] or is_subset[j]:
                    continue
                if companies_dict[i] < companies_dict[j]:
                    is_subset[i] = 1
                if companies_dict[j] < companies_dict[i]:
                    is_subset[j] = 1
        return [i for i in range(length) if is_subset[i] == 0]
