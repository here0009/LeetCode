"""
绘制直线。有个单色屏幕存储在一个一维数组中，使得32个连续像素可以存放在一个 int 里。屏幕宽度为w，且w可被32整除（即一个 int 不会分布在两行上），屏幕高度可由数组长度及屏幕宽度推算得出。请实现一个函数，绘制从点(x1, y)到点(x2, y)的水平线。

给出数组的长度 length，宽度 w（以比特为单位）、直线开始位置 x1（比特为单位）、直线结束位置 x2（比特为单位）、直线所在行数 y。返回绘制过后的数组。

示例1:

 输入：length = 1, w = 32, x1 = 30, x2 = 31, y = 0
 输出：[3]
 说明：在第0行的第30位到第31为画一条直线，屏幕表示为[0b000000000000000000000000000000011]
示例2:

 输入：length = 3, w = 96, x1 = 0, x2 = 95, y = 0
 输出：[-1, -1, -1]

来源：力扣（LeetCode）
链接：https://leetcode-cn.com/problems/draw-line-lcci
著作权归领扣网络所有。商业转载请联系官方授权，非商业转载请注明出处。
"""


from typing import List
class Solution:
    def drawLine(self, length: int, w: int, x1: int, x2: int, y: int) -> List[int]:
        maxtrix = [['0'] * 32 for _ in range(length)]
        res = []
        start, end = x1 + y * w, x2 + y * w
        # print(start, end)
        for idx in range(start, end + 1):
            i, j = divmod(idx, 32)
            maxtrix[i][j] = '1'
        # for row in maxtrix:
        #     print(''.join(row))
        for row in maxtrix:
            sign = row[0]
            if sign == '0':
                res.append(int(''.join(row[1:]), 2))
            else:
                vals = ['1' if v == '0' else '0' for v in row[1:]]
                v = int(''.join(vals), 2)
                v += 1
                string = bin(v)[-31:]
                res.append(-int(string, 2))
        return res


class Solution:
    def drawLine(self, length: int, w: int, x1: int, x2: int, y: int) -> List[int]:
        maxtrix = [['0'] * 32 for _ in range(length)]
        res = []
        start, end = x1 + y * w, x2 + y * w
        one_31 = (1 << 31) - 1
        for idx in range(start, end + 1):
            i, j = divmod(idx, 32)
            maxtrix[i][j] = '1'
        # for row in maxtrix:
        #     print(''.join(row))
        for row in maxtrix:
            sign, val = row[0], int(''.join(row[1:]), 2)
            if sign == '0':
                res.append(val)
            else:
                # print(val, bin(val))
                val = (val ^ one_31) + 1
                # print(val, bin(val))
                res.append(-val)
        return res

S = Solution()
length = 1
w = 32
x1 = 30
x2 = 31
y = 0
print(S.drawLine(length, w, x1, x2, y))
length = 3
w = 96
x1 = 0
x2 = 95
y = 0
print(S.drawLine(length, w, x1, x2, y))
print(S.drawLine(15,96,81,95,1))
print(S.drawLine(2, 64, 0, 52, 0))
# 输出：
# [-1,4294965248]
# 预期结果：
# [-1,-2048]